...
__attribute__((device)) int warp_bcast(int v, int leader) {
	return __shfl_sync(0xffffffff, v, leader);
}
		...
		unsigned int* m_ncand)
{
	...
	int in_off =  array4d_idx8(nbeams, 1, ndt, ndt, ibeam, 0, idt, 0); // input offset
	int out_off = array4d_idx8(nbeams, ndt, nt, NBOX, ibeam, idt, 0, 0); // output offset
	int hist_off =  array4d_idx8(1, nbeams, ndt, NBOX, 0, ibeam, idt, 0); // history offset
	...
	for(int t = 0; t < nt; ++t) {
		...
		if (cand.t >= 0) {
			...
			if (::__all_sync(0xffffffff, vout < threshold) || t == nt - 1) { // if all boxcars are below threshold, or we're at the end of the block
				...
				int boxcar_mask = __ballot_sync(0xffffffff, best_sn_for_ibc == cand.sn);
			...
			if (::__any_sync(0xffffffff, vout >= threshold)) { // one of the boxcars has a detection that beats the threshold
				cand.sn = vout;
				cand.t = t;
			}
		}
		...
		unsigned int* m_ncand)
{
	...
	int ibeam = blockIdx.x; // beam index
	int grid_dt = blockIdx.y; // delta_t of grid
	int thread_dt = threadIdx.y; // delta_t from threax
	int idt = thread_dt + DT_BLOCKS*grid_dt; // total delta_t
	int ibc = threadIdx.x; // boxcar width in samples. ibc=0 is 1 sample wide.
	...
	for(int t = 0; t < nt; ++t) {
		...
		vprev = __shfl_up_sync(vprev, 1, NBOX);
		...
		if (cand.t >= 0) {
			...
			if (::__all_sync(0xffffffff, vout < threshold) || t == nt - 1) { // if all boxcars are below threshold, or we're at the end of the block
				...
				// work out which ibc has the best vout - do a warp ballot of which ibc owns the best one
				int boxcar_mask = __ballot_sync(0xffffffff, best_sn_for_ibc == scaled_sn);
				int best_ibc = __ffs(boxcar_mask) - 1; // __ffs finds first set bit = lowsest ibc that had the all tiem best vout
		...
		int maxbc)
{
	...
	int in_off = array4d_idx8(nbeams, 1, ndt, ndt + nt, ibeam, 0, idt, 0); // input offset
	int out_off =  array4d_idx8(nbeams, ndt, nt, NBOX, ibeam, idt, 0, 0); // output offset
	int hist_off =  array4d_idx8(1, nbeams, ndt, NBOX, 0, ibeam, idt, 0); // history offset
	int discard_off = array4d_idx8(1,1,nbeams,ndt,0,0,ibeam,idt); // discard offset
	...
	for(int blk = 0; blk < nt/NBOX; ++blk) {
		...
		for (int t = 0 ; t < NBOX; ++t) {
			...
			if (cand.t >= 0) {
				...
				// Find out if the candidate ended this sample: do warp vote to find out if all boxcars are now below threshold
				if (::__all_sync(0xffffffff, vout < threshold) || fullt == nt - 1) { // if all boxcars are below threshold, or we're at the end of the block
					// find maximum across all boxcars
					fdmt_dtype scaled_sn = cand.sn/ibc_scale; // scale by boxcar width, so all boxcars have the same variance
					fdmt_dtype best_sn_for_ibc = warpAllReduceMax(scaled_sn);
					// work out which ibc has the best vout - do a warp ballot of which ibc owns the best one
...
#pragma omp parallel for shared(ncand, inp, outp)
	for(int b = 0; b < nbeams; ++b) {
		...
		for(int idt = mindm; idt < ndt; ++idt) {
			...
			int histidx = 0;//array4d_idx(boxcar_history, 0, b, idt, 0);
			...
			for(int t = 0; t < nt; ++t) {
				int inidx = 0;//array4d_idx(indata, b, 0, idt, t);
