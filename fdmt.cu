	...
	for(int beam = 0; beam < fdmt->nbeams; beam++) {
		...
		for (int c = 0; c < fdmt->nf; c++) {
			...
			for (int idt = 1; idt < fdmt->delta_t; idt++) {
				int outidx = 0;//array4d_idx(state, beam, c, idt, 0);
				int iidx = 0;//array4d_idx(state, beam, c, idt-1, 0);
		...
		fdmt_dtype* __restrict__ state, int delta_t, int max_dt, int nt)
{
	...
	// Assign initial data to the state at delta_t=0
	int outidx = array4d_idx8_(nbeams, nf, delta_t, max_dt, ibeam, c, 0, 0);
	int imidx =array4d_idx8_(nbeams, nf, 1, nt, ibeam, c, 0, 0);
	for (int t = 0; t < nt; ++t) {
	...
	for (int idt = 1; idt < delta_t; ++idt) {
		int outidx =  array4d_idx8_(nbeams, nf, delta_t, max_dt, ibeam, c, idt, 0);
		int iidx   = array4d_idx8_(nbeams, nf, delta_t, max_dt, ibeam, c, idt-1, 0);
		int imidx =  array4d_idx8_(nbeams, nf, 1, nt, ibeam, c, 0, nt - 1);
		...
		fdmt_dtype* __restrict__ state, int delta_t, int max_dt, int nt, bool count)
{
	...
	// Assign initial data to the state at delta_t=0
	int outidx = array4d_idx8_(nbeams, nf, delta_t, delta_t + nt, ibeam, c, 0, 0);
	int imidx = array4d_idx8_(nbeams, nf, 1, nt, ibeam, c, 0, 0);
	while (t < nt) {
		if (count) {
			state[outidx + t] = 1.;
	...
	// Do partial sums initialisation recursively (Equation 20.)
	for (int idt = 1; idt < delta_t; ++idt) {
		int outidx =array4d_idx8_(nbeams, nf, delta_t, delta_t + nt, ibeam, c, idt, 0);
		int iidx   = array4d_idx8_(nbeams, nf, delta_t, delta_t + nt, ibeam, c, idt-1, 0);
		int imidx  =array4d_idx8_(nbeams, nf, 1, nt, ibeam, c, 0, 0 );
										...
										int nt)
{
	...
	int off = array4d_idx8_(1, nbeams, max_dt, max_dt+nt, 0, ibeam, idt, 0);
...
__host__ void fdmt_copy_valid_ostate2(const fdmt_t* fdmt, array4d_t* out)
{
	...
	for(int b = 0; b < fdmt->nbeams; ++b) {
		int inidx = 0;//array4d_idx(&fdmt->ostate, b, 0, 0, 0);
		int oidx = 0;//array4d_idx(out, 0, b, 0, 0);
...
//------------------------------------
__host__ __device__ int array4d_idx8_(int nw, int nx, int ny, int nz, int w, int x, int y, int z)
{
