...
float fdmt::cff(float f1_start, float f1_end, float f2_start, float f2_end) {
 ...
 SizeType fdmt::calculate_dt_sub(float f_start, float f_end, float f_min,
                              float f_max,  SizeType dt) {
    float ratio = cff(f_start, f_end, f_min, f_max);
    return static_cast< SizeType>(std::round(static_cast<float>(dt) * ratio));
}

void fdmt::add_offset_kernel(const float* arr1,  SizeType size_in1,
                             const float* arr2,  SizeType size_in2, float* arr_out,
                              SizeType size_out,  SizeType offset)
{
    ...
    t_ind += nsum;
     ...
     SizeType nrest = std::min(offset, size_out - t_ind);
    if (nrest > 0) 
    {
...
void fdmt::copy_kernel(const float* arr1,  SizeType size_in, float* arr_out, SizeType size_out) 
{
    if (size_out < size_in)
    {
        throw std::runtime_error("Output size is less than input size");
